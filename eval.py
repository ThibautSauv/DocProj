...
def padImg(img):
    ...
    padImg = cv2.copyMakeBorder(
        padImg, padding, padding, padding, padding, cv2.BORDER_REPLICATE
    )
...
def cropToPatch(img):
    ...
    for j in range(0, ynum):
        for i in range(0, xnum):
            ...
            gy = int(y - padding)
            globalpatch = img[
                gy : int(gy + globalFct * patchRes), gx : int(gx + globalFct * patchRes)
            ]
...
def execute(imgPath, modelPath, saveImgPath, saveFlowPath):
    img = resizeImg(imgPath, H=2000, W=1500)
    io.imsave(saveImgPath, img_as_ubyte(img))
    img = padImg(img)
    totalLocalPatch, totalGlobaPatch = cropToPatch(img)
